...
private:
        ...
        T fixed_block(DW_FORM form)
        {
                ...
                case DW_FORM::block4:
                        ...
                        break;
                default:
                        throw value_type_mismatch("cannot read " + to_string(form) + " as block");
                }
                ensure(len);

                T val = 0;
                memcpy(&val, pos, ::std::min(sizeof(val), len));
                pos += len;
                return val;
        }
...
/**
 * Advanced cursor to resolve cross-section references.
 */
class dwarf_cursor : public cursor
{
public:
        dwarf_cursor(const dwarf &file, const std::shared_ptr<section> sec, section_offset offset = 0)
                : cursor(sec, offset), m_dwarf(file)
        { }

        using cursor::cstr;
        const char *cstr(DW_FORM form);
...
struct attribute_spec
{
        ...
        attribute_spec(DW_AT name, DW_FORM form, int64_t val = 0);
};
