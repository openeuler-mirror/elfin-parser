 ...
 * also records the set of source file_list for a given compilation unit,
...
public:
         ...
         */
        line_table(const compilation_unit &cu, const std::shared_ptr<section> &sec, section_offset offset);
        ...
        class format_entry;
        class format;
        class directory_format;
        class file_format;
        class directory;
        class file;
        template<typename ItemT> class path_list;
...
/**
 * A format in a line table.
 */
class line_table::format : protected std::vector<format_entry>
{
public:
        using std::vector<format_entry>::const_iterator;

        using std::vector<format_entry>::vector;

        using std::vector<format_entry>::size;
        using std::vector<format_entry>::front;
        using std::vector<format_entry>::cbegin;
        using std::vector<format_entry>::begin;
        using std::vector<format_entry>::cend;
        using std::vector<format_entry>::end;

        virtual void init(cursor &cur) = 0;
};

/**
 * A file format in a line table.
 */
class line_table::file_format : public format
{
public:
        file_format() = default;

        void init(cursor &cur) override;
};

/**
 * A directory format in a line table.
 */
class line_table::directory_format : public format
{
public:
        directory_format() = default;
...
/**
 * A directory in a line table.
 */
class line_table::directory
{
public:
        directory() = default;
        directory(const ::std::string &path);
        directory(const ::std::string &path, const ::std::string &comp_dir);
        directory(dwarf_cursor &cur, const ::std::string &comp_dir); // DWARF4
        directory(dwarf_cursor &cur, const format &format); // DWARF5
...
protected:
        ...
        void validate_path();
        void validate_path(const ::std::string &comp_dir);
};

/**
 * A source file in a line table.
 */
class line_table::file : public line_table::directory
{
public:
...
/**
 * A directory list in a line table.
 */
